...
def get_endpoints(endpoints_path, save_path, **context):
...
t3 = PythonOperator(task_id = "get_endpoints",
                    python_callable = get_endpoints,
                    op_args = [ENDPOINT_PATH, SAVE_PATH],
                    dag = dag)
...
t4 = FileToGoogleCloudStorageOperator(
                    ...
                    bucket = "{{var.value.gcs_ps_mirror}}",
                    src =  "{{ task_instance.xcom_pull(task_ids='get_endpoints', key='file_path' )}}",


                    dst = "powerschool/" + endpoint_name + "/{{ task_instance.xcom_pull(task_ids='get_endpoints', key='file_name') }}",
                    dag = dag
)
